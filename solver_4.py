...
class excitation_solver:
    ...
    def trions_3_electrons_explicit(self, flat_bands, spins):
        # this is the explicitely calculated matrix elements for a spin up, down, down only (or down, up, up)
                                    ...
                                    for l in range(self._N_f):
                                        for k_1 in range(self._N):
                                            for k_2 in range(self._N):
                                                ...
                                                if k_1==k_1_prime:
                                                    R_sum-=(np.cos((p+k_1)%N*2*np.pi/N)+np.cos((p+k_1+k_2+k_2_prime)%N*2*np.pi/N))
                                                    R_sum-=(1+np.cos((k_2-k_2_prime)%N*2*np.pi/N))
                                                    
                                                if k_2==k_2_prime:
                                                    R_sum-=(np.cos((p+k_2)%N*2*np.pi/N)+np.cos((p+k_1+k_2+k_1_prime)%N*2*np.pi/N))
                                                    R_sum-=(1+np.cos((k_1-k_1_prime)%N*2*np.pi/N))
                                                
                                                if k_2_prime==k_1:
                                                    R_sum+=(np.cos((p+k_1)%N*2*np.pi/N)+np.cos((p+k_1+k_2+k_1_prime)%N*2*np.pi/N))
                                                    R_sum+=(1+np.cos((k_2-k_1_prime)%N*2*np.pi/N))
                                                    
                                                if k_1_prime==k_2:
                                                    R_sum+=(np.cos((p+k_2)%N*2*np.pi/N)+np.cos((p+k_1+k_2+k_2_prime)%N*2*np.pi/N))
                                                    R_sum+=(1+np.cos((k_1-k_2_prime)%N*2*np.pi/N))
                                                    
                                                self._R[:,R_row_index,R_column_index] += 0.25*R_sum / N
    ...
    def charge_4_electrons(self, flat_bands,spins):
